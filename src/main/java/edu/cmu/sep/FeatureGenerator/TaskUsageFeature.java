...
public class TaskUsageFeature extends Feature {
    ...
    public void generateFeatureSingleValue(String[] tableRowArray) {
        ...
        String startTime = tableRowArray[mTableSchema.indexOf("start time")].length() == 0? "0" : tableRowArray[mTableSchema.indexOf("start time")];
        String endTime = tableRowArray[mTableSchema.indexOf("end time")].length() == 0? "0" : tableRowArray[mTableSchema.indexOf("end time")];
        String cpuRate = tableRowArray[mTableSchema.indexOf("CPU rate")].length() == 0? "0" : tableRowArray[mTableSchema.indexOf("CPU rate")];
        String canonicalMemUsage = tableRowArray[mTableSchema.indexOf("canonical memory usage")].length() == 0? "0" : tableRowArray[mTableSchema.indexOf("canonical memory usage")];
        String assignedMemUsage = tableRowArray[mTableSchema.indexOf("assigned memory usage")].length() == 0? "0" : tableRowArray[mTableSchema.indexOf("assigned memory usage")];
        String unmappedMemUsage = tableRowArray[mTableSchema.indexOf("unmapped page cache")].length() == 0? "0" : tableRowArray[mTableSchema.indexOf("unmapped page cache")];
        String totalPageCache = tableRowArray[mTableSchema.indexOf("total page cache")].length() == 0? "0" : tableRowArray[mTableSchema.indexOf("total page cache")];
        String maxMemUsage = tableRowArray[mTableSchema.indexOf("maximum memory usage")].length() == 0? "0" : tableRowArray[mTableSchema.indexOf("maximum memory usage")];
        String diskIOTime = tableRowArray[mTableSchema.indexOf("disk I/O time")].length() == 0? "0" : tableRowArray[mTableSchema.indexOf("disk I/O time")];
        String localDiskSpaceUsage = tableRowArray[mTableSchema.indexOf("local disk space usage")].length() == 0? "0" : tableRowArray[mTableSchema.indexOf("local disk space usage")];
        String maxCPURate = tableRowArray[mTableSchema.indexOf("maximum CPU rate")].length() == 0? "0" : tableRowArray[mTableSchema.indexOf("maximum CPU rate")];
        String maxDiskIOTime = tableRowArray[mTableSchema.indexOf("maximum disk IO time")].length() == 0? "0" : tableRowArray[mTableSchema.indexOf("maximum disk IO time")];
        ...
        if (newRecord) {
            ...
            String jobDuration = Long.toString(Long.parseLong(endTime) - Long.parseLong(startTime));
            values.add(jobDuration);
        } else {
            String jobDuration = Long.toString(Long.parseLong(values.get(0)) + Long.parseLong(endTime) - Long.parseLong(startTime));
            values.set(0, jobDuration);
        }
