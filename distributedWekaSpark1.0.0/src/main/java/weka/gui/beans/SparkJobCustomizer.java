...
public class SparkJobCustomizer extends JPanel implements BeanCustomizer,
  ...
  protected void setup() {
    ...
    if (m_job instanceof weka.distributed.spark.ArffHeaderSparkJob) {
      ...
      addTabForEvaluationJob(jobTitle, m_job);
    } else if (m_job instanceof weka.distributed.spark.WekaAttributeSelectionSparkJob) {
        m_tempArffJob = new weka.distributed.spark.ArffHeaderSparkJob();
        try {
          m_tempArffJob.setOptions(Utils.splitOptions(m_optionsOrig));
        } catch (Exception ex) {
        ...
        addTabForArffHeaderJob("ARFF header/CSV parsing", m_tempArffJob);
        addTabForAttrSelJob(jobTitle, m_job);
  ...
  /**
   * Adds a tab for editing an attribute selection job
   *
   * @param tabTitle the title for the tab
   * @param evaluationJob the evaluation job to edit
   */
  public void addTabForAttrSelJob(String tabTitle, SparkJob attrSelJob) {
    JPanel jobHolder = new JPanel();
    jobHolder.setLayout(new BorderLayout());

    PropertySheetPanel attrSelJobEditor = new PropertySheetPanel();
    attrSelJobEditor.setEnvironment(m_env);
    attrSelJobEditor.setTarget(attrSelJob);
    jobHolder.add(attrSelJobEditor, BorderLayout.NORTH);
    ...
    JPanel attrSelJobP = makeClassifierJobPanel(m_tempClassifierJob, true);

    jobHolder.add(attrSelJobP, BorderLayout.CENTER);
    JScrollPane scroller = new JScrollPane(jobHolder);
  ...
  /**
   * Add options from the attr selection job only to the supplied list
   *
   * @param opts the list of options to add to
   */
  protected void addAttrSelJobOptionsOnly(List<String> opts) {
    String[] evalOpts = ((weka.distributed.spark.WekaAttributeSelectionSparkJob) m_job)
      .getJobOptionsOnly();
   ...
   * condition
   */
  protected void okAttrSelJob() {
    List<String> opts = getBaseConfig(m_tempClassifierJob);
    addArffJobOptionsOnly(opts, m_tempArffJob);
    addArffMapTaskOpts(opts);
    addAttrSelJobOptionsOnly(opts);
